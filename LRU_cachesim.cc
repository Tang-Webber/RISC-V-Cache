...
uint64_t* cache_sim_t::check_tag(uint64_t addr)
{
  ...
  uint64_t* tags_ptr = tags;

  for (size_t i = 0; i < ways; i++){
    if (tag == (tags[idx*ways + i] & ~DIRTY)){
      auto partitionPoint = std::stable_partition(V[idx].begin(), V[idx].end(), [i](const auto& element) {
...
uint64_t cache_sim_t::victimize(uint64_t addr)
{
  ...
  if(n[idx] < ways){
    ...
    V[idx].push_back(std::make_pair(n[idx], value));
    n[idx]++;
  }
  else{
    ...
    V[idx].push_back(std::make_pair(front.first, value));
