...
size_t n[16] = {0};
std::vector<std::vector<std::pair<size_t, uint64_t>>> V(16);
...
uint64_t* cache_sim_t::check_tag(uint64_t addr)
{
  ...
  std::stable_sort(V[idx].begin(), V[idx].end(), [](const std::pair<size_t, uint64_t>& a, const std::pair<size_t, uint64_t>& b) {
...
uint64_t cache_sim_t::victimize(uint64_t addr)
{
  ...
  if(n[idx] < ways){
    tags[idx*ways + n[idx]] = value;
    V[idx].push_back(std::make_pair(n[idx], 1));
    n[idx]++;
  }
  else{ 
    std::pair<size_t, uint64_t> front = V[idx].front();
    victim = tags[idx*ways + front.first];
    V[idx].erase(V[idx].begin());
    V[idx].push_back(std::make_pair(front.first, 1));
    tags[idx*ways + front.first] = (addr >> idx_shift) | VALID;
  }
...
{    
  std::stable_sort(W.begin(), W.end(), [](const std::pair<uint64_t, uint64_t>& a, const std::pair<uint64_t, uint64_t>& b) {
      return a.second < b.second;
  }); 
  uint64_t old_tag = 0;
  if (tags.size() == ways)
  ...
  W.push_back(std::make_pair((addr >> idx_shift), 1));  
  return old_tag;
}
